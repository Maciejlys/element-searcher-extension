...
describe('handleFormSubmit', () => {
  ...
  beforeEach(() => {
    ...
    executeScriptSpy = vi
      .spyOn(global.chrome.scripting, 'executeScript')
      .mockResolvedValue();
    queryTabSpy = vi
      .spyOn(global.chrome.tabs, 'query')
  ...
  test('Should execute script with proper tab id', async () => {
    ...
    expect(args).toEqual([['a'], ColorList]);
  });
  ...
  test('Should pass proper selectors as argument', async () => {
    ...
    const [executeScriptArgs] = executeScriptSpy.mock.calls[0];
    expect(executeScriptArgs.args).toEqual([expectedArgs, ColorList]);
  });
  ...
  test('Should inject text into amountFound', async () => {
    ...
    callback([{ result: 123 }]);
    expect(fakeAmountFoundElement.innerText).toEqual(
      '123 elements found with given selectors',
    );
  });
});
